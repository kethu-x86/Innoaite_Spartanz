...
class TrafficController:
    ...
    def _load_models(self):
        ...
        self.lstm_model.load_state_dict(
            torch.load(self.base_path / "lstmv1.pth", map_location=self.device)
        )
        ...
        self.dqn.load_state_dict(
            torch.load(
                self.base_path / "dqn_active_model.pth", map_location=self.device
            )
        )
        self.dqn.eval()
    ...
    def get_action(
        self, current_counts, sim_time=None, current_phase=0, emergency_direction=None
    ):
        """
        ...
        if emergency_direction:
            ...
            if emergency_direction in green_dirs:
                logger.info(
                    f"🚨 Emergency: {emergency_direction} already has green (phase {current_phase}). Keeping."
                )
                return 0  # Already green for emergencydirection
            else:
                logger.info(
                    f"🚨 Emergency: Switching to give {emergency_direction} green (from phase {current_phase})."
                )
        ...
        try:
            with torch.no_grad():
                ...
                if hasattr(self.scaler_x, "feature_names_in_"):
                    scaled_seq = self.scaler_x.transform(
                        pd.DataFrame(
                            history_list, columns=self.scaler_x.feature_names_in_
                        )
                    )
                else:
                ...
                state = np.concatenate([features, preds * 0.3, [current_phase]]).astype(
                    np.float32
                )
                ...
                self.latest_metrics = {
                    ...
                    "predicted_congestion_index": float(
                        preds.mean()
                    ),  # Simple aggregate of LSTM output
                }
        ...
        try:
            for direction, sensors in sensor_map.items():
                for sensor_id in sensors:
                    counts[direction] += traci.inductionloop.getLastStepVehicleNumber(
                        sensor_id
                    )
        except Exception as e:

...
class StreamGenerator:
    ...
    def generate(self):
        ...
        while True:
            ...
            if cap == "dummy" or (
                isinstance(cap, cv2.VideoCapture) and not cap.isOpened()
            ):
                if cap != "dummy":
                    logger.warning(
                        f"Source {self.current_source_idx} ({cam_id}) is not opened, falling back to dummy frames"
                    )
                ...
                cv2.circle(
                    frame,
                    (
                        int(time.time() * 100) % self.target_size[0],
            ...
            if not ret:
                ...
                if isinstance(cap, cv2.VideoCapture) and cap.isOpened():
                    logger.info(
                        f"Source {self.current_source_idx} ended, restarting..."
                    )
            ...
            yield {"frame": frame, "cam_id": cam_id, "batch_id": self.batch_id}
...
if __name__ == "__main__":
    ...
    try:
        for data in gen.generate():
            ...
            cv2.putText(
                frame, text, (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
            )
